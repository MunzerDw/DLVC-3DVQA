""" 
Modified from: https://github.com/ATR-DBI/ScanQA/blob/main/models/nms.py
"""
...
def nms_3d_num_proposals(boxes, num_proposals, old_type=False):
    x1 = boxes[:,0]
    y1 = boxes[:,1]
    z1 = boxes[:,2]
    x2 = boxes[:,3]
    ...
    for s in I:
        ...
        xx1 = np.maximum(x1[s], x1[I[i+1:]])
        yy1 = np.maximum(y1[s], y1[I[i+1:]])
        zz1 = np.maximum(z1[s], z1[I[i+1:]])
        xx2 = np.minimum(x2[s], x2[I[i+1:]])
        yy2 = np.minimum(y2[s], y2[I[i+1:]])
        zz2 = np.minimum(z2[s], z2[I[i+1:]])
        ...
        if old_type:
            ...
            inter = l*w*h
            o = inter / (area[i] + area[I[i+1:]] - inter)
        indices = np.array(range(i+1, len(I)))[...,None]
        o = o[...,None]
        #print(f"o Shape: {o.shape}")
        #print(f"indices Shape: {indices.shape}")
        overlaps.extend(np.concatenate([o,indices],1))
        i +=1
    overlaps = sorted(overlaps, key = lambda x: x[0])[::-1]
    num_removals = len(I) - num_proposals
    del_boxes = []
    for el in overlaps:
        if el[1] not in del_boxes:
            del_boxes.append(el[1])
            num_removals -= 1
            if num_removals == 0:
                break
    
    pick = I[~np.isin(I,del_boxes)]
