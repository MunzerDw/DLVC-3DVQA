"""
Added additional answer loss,
all other losses modified from https://github.com/daveredrum/ScanRefer/blob/master/lib/loss_helper.py
"""
...
from .losses import  SoftmaxRankingLoss
from utils.box_util import get_3d_box, get_3d_box_batch, box3d_iou, box3d_iou_batch
from lib.visualize import dump_results, write_bbox, write_ply_rgb, write_ply
FAR_THRESHOLD = 0.6
NEAR_THRESHOLD = 0.3
GT_VOTE_FACTOR = 3 # number of GT votes per point
OBJECTNESS_CLS_WEIGHTS = [0.2, 0.8] # put larger weights on positive objectness

# Compute Loss for generated Answer
def compute_answer_loss(data_dict):
    # output
    out_combined = data_dict['out_combined']
...
def compute_vote_loss_vn(data_dict):
...
def compute_objectness_loss_vn(data_dict):
...
def compute_box_and_sem_cls_loss_vn(data_dict, config):
...
def compute_reference_loss_vn(data_dict, config):
...
def compute_lang_classification_loss_vn(data_dict):
    loss_lang = F.cross_entropy(data_dict["lang_scores"], data_dict["object_cat"])
    return loss_lang


def compute_answer_classification_loss_vn(data_dict):
    """ Compute cluster reference loss
...
def get_loss_vn(data_dict, config, detection=True, use_reference=True, use_lang_classifier=True, use_answer=True, loss_weights=None, scorer = None, vocab=None, embs=None):
    """ Loss functions
    ...
    # Vote loss
    vote_loss = compute_vote_loss_vn(data_dict)

    # Obj loss
    objectness_loss, objectness_label, objectness_mask, object_assignment = compute_objectness_loss_vn(data_dict)
    ...
    center_loss, heading_cls_loss, heading_reg_loss, size_cls_loss, size_reg_loss, sem_cls_loss = compute_box_and_sem_cls_loss_vn(data_dict, config)
    ...
    if use_reference:
        # Reference loss
        ref_loss, _, cluster_labels = compute_reference_loss_vn(data_dict, config)
        ...
        # Answer loss
    data_dict['answer_loss'] = compute_answer_loss(data_dict)


    #if reference and use_lang_classifier:
    if use_lang_classifier:
        data_dict["lang_loss"] = compute_lang_classification_loss_vn(data_dict)
    else:
        data_dict["lang_loss"] = torch.zeros(1)[0].cuda()

    loss = loss_weights.get('vote_loss', 1.) * data_dict['vote_loss'] \
            + loss_weights.get('objectness_loss', 0.5) * data_dict['objectness_loss'] \
            + loss_weights.get('box_loss', 1.) * data_dict['box_loss'] \
            + loss_weights.get('sem_cls_loss', 0.1) * data_dict['sem_cls_loss'] \
            + loss_weights.get('ref_loss', 0.1) * data_dict["ref_loss"] \
            + loss_weights.get('lang_loss', 0.1) * data_dict["lang_loss"] \
            + loss_weights.get('answer_loss', 1.) * data_dict['answer_loss'] \

    loss *= 10 # amplify
    losses = {
      'lang_loss': data_dict['lang_loss'],
      'answer_loss': data_dict['answer_loss'],
      'vote_loss': vote_loss,
      'objectness_loss': objectness_loss,
      'box_loss': box_loss,
      'sem_cls_loss_sum': sem_cls_loss,
      'ref_loss': data_dict['ref_loss']
    }
    data_dict['loss'] = loss
    return loss, data_dict, losses