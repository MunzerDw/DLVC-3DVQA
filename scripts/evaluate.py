...
# evaluation
def evaluate(dataset, model, device=None):
    ...
    for batch in tqdm(batch_iter):
        ...
        for i in range(pred_ref.shape[0]):
            ...
            gt_bboxes.append(gt_bbox_batch[i])
    ...
    for i, scene_id in enumerate(scene_ids):
        ...
        preds_pickle[dict['scene_id']][dict['question_id']]['gt_answers'] = dict['gt_answers']
...
def main():
    ...
    # get predictions
    print('evaluating...')
    preds, preds_pickle = evaluate(val_dataset, model, device=DEVICE)

    # save predictions to files
    pred_path = os.path.join(CONF.PATH.OUTPUT, experiment, version, "pred.val.pkl")
