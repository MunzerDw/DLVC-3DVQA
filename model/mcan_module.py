...
# Our trial change to MCAN
# class MCAN_ED(nn.Module):
#     def __init__(self, hidden_size, num_heads=8, num_layers=2, pdrop=0.1):
#         super(MCAN_ED, self).__init__()
        
#         self.num_layers = num_layers
        
#         self.self_attn_lang = SA(hidden_size, num_heads, pdrop)
        
#         self.self_attn_list = nn.ModuleList([SA(hidden_size, num_heads, pdrop) for _ in range(num_layers)])
#         self.cross_attn_list = nn.ModuleList([CA(hidden_size, num_heads, pdrop) for _ in range(num_layers)])

#     def forward(self, x, y, x_mask, y_mask):
        
#         # self attention            
#         x = self.self_attn_lang(x, x_mask)
#         y = self.self_attn_list[0](y, y_mask)
        
#         # cross attention
#         x = self.cross_attn_list[0](x, y, x_mask, y_mask)
#         y = self.cross_attn_list[0](y, x, y_mask, x_mask)
        
#         for _ in range(self.num_layers - 1):            
#             # self attention
#             x = self.self_attn_list[_+1](x, x_mask)
#             y = self.self_attn_list[_+1](y, y_mask)

#             # cross attention
#             x = self.cross_attn_list[_+1](x, y, x_mask, y_mask)
#             y = self.cross_attn_list[_+1](y, x, y_mask, x_mask)
