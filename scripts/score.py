"""
Modified from https://github.com/ATR-DBI/ScanQA/blob/main/scripts/score.py
"""
...
if __name__=="__main__":
    ...
    folder = args.experiment + "/version_" + str(args.version)
    ...
    if args.detailed:
        for split in  ['val']:
            ...
            if split in ['val', 'test_w_obj']:
                fin=os.path.join(CONF.PATH.OUTPUT, folder, f'pred.{split}.pkl')
                ...
                fin=open(os.path.join(CONF.PATH.OUTPUT, folder, 'pred.json'))
    ...
    # val
    #
    fin=os.path.join(CONF.PATH.OUTPUT, folder, 'pred.val.pkl')
    preds=pickle.load(open(fin,'rb'))
    preds={qid:value for scene,pred_scenes in preds.items() for qid,value in pred_scenes.items()} # simplify data format to dict[qid,pred]
    for _,pred in preds.items():
        pred['answer_top10']=pred['pred_answers_at10']
