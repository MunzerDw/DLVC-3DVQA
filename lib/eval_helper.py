...
from lib.losses import SoftmaxRankingLoss
...
def get_eval(data_dict, config, answer_vocab=None, use_reference=False, use_lang_classifier=False, use_oracle=False, use_cat_rand=False, use_best=False, post_processing=None):
    ...
    accuracies = {
        "ref_acc": np.mean(data_dict["ref_acc"]),
        "lang_acc": data_dict["lang_acc"].item(),
        "obj_acc" : data_dict["obj_acc"].item(),
        "pos_ratio": data_dict["pos_ratio"].item(),
        "neg_ratio": data_dict["neg_ratio"].item(),
        "iou_rate_0.25": np.mean(data_dict["ref_iou_rate_0.25"]),
        "iou_rate_0.5": np.mean(data_dict["ref_iou_rate_0.5"])
    }
    return data_dict, accuracies
